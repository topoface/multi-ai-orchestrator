...
**경고**: 엣지 케이스, 데이터 검증, 보안 세부사항 등 구현 디테일로 발산하지 마세요. 원래 질문의 핵심 선택(RAG vs 파인튜닝)에 집중하세요!
...
**경고**: 엣지 케이스, 데이터 검증, 보안 세부사항 등 구현 디테일로 발산하지 마세요. 원래 질문의 핵심 선택에 집중하세요!
...
2. **합의 여부**: 두 제안이 서로 일치하는가?
3. **실행 가능성**: 구체적이고 실행 가능한가?
4. **핵심 쟁점 해결**: 원래 질문의 핵심 쟁점에 결론이 있는가?

**중요**: 원래 질문과 무관한 세부 구현으로 발산한 경우 반드시 REJECT하고, 원래 질문으로 돌아가도록 유도하세요!
